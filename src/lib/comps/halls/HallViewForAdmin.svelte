<script lang="ts">
	...
	import type { HallReadDto, HallUpdateDto, SeatReadDto } from "../../../Api";
    ...
    const updateMutation = createMutation({
        ...
        onSuccess: () => {
            client.invalidateQueries({queryKey: ['hall']});
            editHallNameMode = false;
        }
    });
    ...
    <div class="py-5 lg:w-3/4 mx-auto overflow-x-auto">
        <div class="flex justify-start space-x-2">
            {#if !editHallNameMode}
                ...
                <button on:click={(()=> {editHallNameMode=true})} aria-label="edit" class="inline-block rounded-sm bg-transperent px-4 py-2 text-xs font-medium text-cyan-300 hover:text-cyan-100 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill" viewBox="0 0 16 16">
                    <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325"/>
                </svg>
                </button>
            {:else}
                <input bind:value={hallUpdate.name} id="name" type="text" class="w-1/4 mb-4 px-3 py-2 bg-transparent border border-cyan-600 rounded text-cyan-100 focus:outline-none focus:border-cyan-400 placeholder-cyan-500 transition-colors"/>
                ...
                <button on:click={(()=> {editHallNameMode=false})} aria-label="cancel" class="inline-block rounded-sm bg-transperent px-4 py-2 text-xs font-medium text-cyan-300 hover:text-cyan-100 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill" viewBox="0 0 16 16">
